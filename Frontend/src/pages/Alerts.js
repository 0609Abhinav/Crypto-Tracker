...
function CreateAlertModal({ onClose, onCreate, currency }) {
  ...
  const inputStyle = { width: "100%", padding: "10px 14px", background: "var(--bg-primary)", border: "1px solid var(--border)", borderRadius: 10, color: "var(--text-primary)", fontSize: 14, outline: "none", fontFamily: "inherit", boxSizing: "border-box" };
      ...
      <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 420 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {selected?.currentPrice && (
            <div style={{ fontSize: 12, color: "var(--text-muted)", padding: "8px 12px", background: "rgba(99,102,241,0.08)", borderRadius: 8 }}>
              Current price: <strong style={{ color: "var(--accent)" }}>{formatPrice(selected.currentPrice, currency.symbol)}</strong>
            </div>
          )}
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["above", "below"].map((c) => (
                <button key={c} type="button" onClick={() => setCondition(c)} style={{ flex: 1, padding: "9px 0", borderRadius: 10, border: "1px solid", borderColor: condition === c ? "var(--accent)" : "var(--border)", background: condition === c ? "rgba(99,102,241,0.1)" : "transparent", color: condition === c ? "var(--accent)" : "var(--text-secondary)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
                  {c === "above" ? "▲ Above" : "▼ Below"}
                </button>
              ))}
            </div>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", display: "block", marginBottom: 6 }}>Target Price ({currency.symbol}) *</label>
            <input type="number" step="any" min="0" value={targetPrice} onChange={(e) => setTargetPrice(e.target.value)} placeholder="0.00" style={inputStyle} onFocus={(e) => e.target.style.borderColor = "var(--accent)"} onBlur={(e) => e.target.style.borderColor = "var(--border)"} required />
          </div>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 6 }}>🔔 Price Alerts</h1>
          <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>{alerts.length} alert{alerts.length !== 1 ? "s" : ""} set</p>
        </div>
        <Button onClick={() => setShowModal(true)}>+ New Alert</Button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "80px 20px", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 20 }}>
          ...
          <Button onClick={() => setShowModal(true)}>Create Your First Alert</Button>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {alerts.map((a) => (
            <div key={a._id} style={{ background: "var(--bg-card)", border: `1px solid ${a.active ? "var(--border)" : "rgba(255,255,255,0.05)"}`, borderRadius: 14, padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", opacity: a.active ? 1 : 0.5 }}>
              <div style={{ flex: 1, minWidth: 200 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  {a.triggered && <span style={{ fontSize: 11, fontWeight: 700, color: "var(--green)", background: "var(--green-bg)", padding: "2px 8px", borderRadius: 6 }}>✓ Triggered</span>}
                </div>
                <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  Alert when price is <span style={{ color: a.condition === "above" ? "var(--green)" : "var(--red)", fontWeight: 700 }}>{a.condition === "above" ? "▲ above" : "▼ below"}</span> <strong style={{ color: "var(--text-primary)" }}>{formatPrice(a.targetPrice, currency.symbol)}</strong>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button onClick={() => handleToggle(a._id)} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: a.active ? "var(--green)" : "var(--text-muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                  {a.active ? "Active" : "Paused"}
                </button>
                <button onClick={() => handleDelete(a._id)} style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 16, padding: 4, borderRadius: 6, transition: "color 0.2s" }}
