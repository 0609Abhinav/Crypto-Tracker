    ...
    <div className="w-full mt-5 bg-gradient-to-br from-blue-100 via-cyan-200 to-blue-300 border shadow-lg rounded-xl p-6 bg-gray-100 hover:scale-105 hover:shadow-2xl transition-transform duration-300 ease-in-out">
      <div className="flex gap-6 items-center">
        <abbr className="w-16" title="Click here to know more.">
          <Link to={`/coin/${item.id}`}>
            <img src={checker === "top15" ? item.thumb : item.image} alt="Crypto symbol" className="rounded-full border-2 border-blue-400" />
          </Link>
        </abbr>
        <div className="flex flex-col w-full">
          <h2 className="text-2xl font-bold text-blue-800">{item.name}</h2>
          <h2 className="text-lg font-semibold text-gray-500 uppercase">{item.symbol}</h2>
        </div>
        {!starRenderingLogic ? (
          <MdOutlineStarRate onClick={addCoin} className="text-[50px] text-yellow-500 cursor-pointer hover:text-yellow-600 transition-colors" />
        ) : (
          <MdOutlineStar onClick={removeCoin} className="text-[50px] text-yellow-500 cursor-pointer hover:text-yellow-600 transition-colors" />
        )}
      </div>

      <div className="flex items-center gap-6 mt-4">
        {checker !== "top15" && (
          <h3 className={`border px-3 py-2 text-lg rounded-3xl font-semibold ${item.price_change_24h > 0 ? "border-green-400 text-green-600" : "border-red-400 text-red-600" } shadow-sm`}>
      ...
      <div className="mt-4 text-sm text-gray-700">
        <div>
          ...
          {checker === "top15" ? item.data.total_volume : item.total_volume}
        </div>
        <div>
          <span className="font-semibold">Market Capacity:</span>{" "}
          {checker === "top15" ? item.data.market_cap : item.market_cap}
        </div>
      </div>
    </div>
  );
